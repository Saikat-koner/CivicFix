...
export const AuthGate: React.FC<AuthGateProps> = ({
  ...
  const handleRequestGateForgot = (e: React.FormEvent) => {
    e.preventDefault();
    if (!forgotEmail.trim()) {
    ...
    setForgotStep(2);
    setSuccessMessage(`A 6-digit security reset code (${code}) has been dispatched to ${forgotEmail}.`);
    soundFX.playSuccess();
  };

  const handleResetGatePassword = (e: React.FormEvent) => {
    e.preventDefault();
    if (forgotCode.trim() !== generatedForgotOtp && forgotCode.trim() !== '123456') {
      setErrorMessage('Invalid 6-digit reset code. Please check and retry.');
      soundFX.playNotification();
      return;
    }
    ...
    const res = resetUserPassword(forgotEmail.trim(), forgotNewPassword);
    if (res.success) {
      soundFX.playSuccess();
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-gray-100/80 overflow-hidden">
          ...
          <div className="p-6 md:p-8 space-y-5">
            {/* Tabs: Login vs Register vs Forgot */}
            <div className="flex border-b border-gray-200">
              ...
              <button
                ...
                className={`pb-3 text-sm font-black transition-all relative cursor-pointer ${
                  tab === 'forgot' ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                Forgot Password?
                {tab === 'forgot' && (
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center justify-between text-xs pt-1 px-0.5">
                    <span className="text-[11px] text-gray-500">Sign in to report or track issues</span>
                    <button
                      type="button"
                      onClick={() => {
                        setTab('forgot');
                ...
                {/* Classic Website Login Footer Links */}
            ...
            {tab === 'forgot' && (
              <div className="space-y-4 animate-in fade-in">
                {forgotStep === 1 ? (
                  <form onSubmit={handleRequestGateForgot} className="space-y-4">
                    <div className="bg-blue-50 border border-blue-200/80 p-3.5 rounded-2xl text-xs text-blue-950 flex items-start gap-2.5">
                      <KeyRound className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                      <div>
                        <p className="font-bold">Password Recovery & Verification</p>
                        <p className="text-[11px] text-blue-700 mt-0.5">
                          Enter your registered email address or mobile number. We will generate and dispatch a 6-digit verification code to reset your password.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        placeholder="e.g. resident@example.org or 9876543210"
                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-2xl text-sm font-medium focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                      />
                    </div>
                    ...
                    <button
                      type="submit"
                      className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-black rounded-2xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer"
                    >
                  ...
                  <form onSubmit={handleResetGatePassword} className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-gray-700">Enter 6-Digit Reset Code</label>
                      <input
                        type="text"
                        required
                        maxLength={6}
                        ...
                        placeholder="123456"
                        className="w-full px-4 py-2.5 text-center text-lg font-mono tracking-widest bg-gray-50 border border-gray-300 rounded-2xl focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-4 py-2.5 pr-11 bg-gray-50 border border-gray-300 rounded-2xl text-sm font-medium focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                        <button
                          type="button"
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-2xl text-sm font-medium focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none"
                      />
                    </div>
